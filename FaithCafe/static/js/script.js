    ...
    try {
        ...
        if (sessionOrdersRaw) {
            try {
                ...
                const map = {};
                jsonOrders.forEach(o => { if (o && o.id) map[o.id] = o; });
                sessionOrders.forEach(o => { if (o && o.id) map[o.id] = o; });
                const merged = Object.values(map);
                sessionStorage.setItem('faithcafe_orders', JSON.stringify(merged));
                console.log('Merged orders from JSON and session cache');
                return merged;
            } catch (e) {
...
async function loginUser(username, password) {
    ...
    // Find by username first to confirm account exists in users.json/session
    ...
    const passwordMatches = (user.password === hashed) || (user.password === password);
...
function displayOrderSummary() {
    ...
    // Calculate shipping fee: Cash = ₱10, Card/GCash = ₱5
    const paymentMethod = document.querySelector('input[name="payment"]:checked');
    let shippingFee = 0;
    if (paymentMethod) {
        if (paymentMethod.value === 'cash') shippingFee = 10;
        else if (paymentMethod.value === 'card' || paymentMethod.value === 'gcash') shippingFee = 5;
    }
